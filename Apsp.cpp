...
void display_choices(char *filename, char *output, char* sorting,int threads,int min,int distribution_method, int mislimit);
...
void APSP(char *filename, char *output, char* sorting,int threads,int min,int distribution_method,int mislimit)
{
  ...
  //  if (method[0]=='0'){
  find_all_pairs(ptr,text,N,K,startS,Sorted,atoi(output),threads,distribution_method,min,mislimit);
    // }else {
...
int main(int argc,char *argv[])
{
 ...
 char *output=new char[2];
  int method,processors,minlength,mismatch;
  // Default values 
  ...
  display_choices(filename,output,sorting,processors,minlength,method,mismatch);
  ...
  APSP(filename,output,sorting,processors,minlength,method,mismatch);
  return 0;
}
...
void displayhelp(){
  ...
  cerr<<"-o\toutput:\n\t 1 for producing an output in an array.\n\t 0 for executing without an output.\n\t 2 for outputing all overlaps (default)."<<endl;
...
void display_choices(char *filename, char *output, char* sorting,int threads,int min,int distribution_method,int mislimit){
  ...
  cerr<<"Number of allowed Mismatches:"<<mislimit<<endl;
