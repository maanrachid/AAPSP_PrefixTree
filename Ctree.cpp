...
void find_all_pairs(struct tree_node *ptr,uchar *T,ulong N,ulong k,ulong startpos[],uint sorted[],int output
		    ,int threads,int distribution_method,int min,int mislimit){
  int **A;
  ...
  */
  ...
  if (threads==1)
     distribute_k_1(threads,T,startpos,sorted,A,ptr,k,N,output,min,mislimit);
...
void distribute_k_1(int threads,uchar *T,ulong startpos[],uint sorted[],int **A,struct  tree_node* ptr,ulong k,ulong N,int output,int min,int mislimit)
{
  omp_set_num_threads(threads);
...
#pragma omp parallel for 
  for(ulong i=0;i<k;i++){
    ...
    while (min<=next-j){	//T[j]!=SEPERATOR
      ...
      all_pairs_helper(v,T,startpos,sorted,A,k,N,output,pos,curpos,curptr,0,i,j,next,mislimit);
      j++;
   }
 }
}
...
void all_pairs_helper(ulong v,uchar *T,ulong startpos[],uint sorted[],int **A,ulong k,ulong N,int output,ulong pos,ulong curpos,struct tree_node *curptr,int mismatch,int i,int j,int next,int mislimit){
	while (1){
			...
			if (pos<=curptr->pos){
			  if (decode(T,v)!=decode(T,startpos[sorted[curptr->from]]+curpos))				mismatch++;
			  if (mismatch>mislimit) return;
			  pos++;curpos++;v++;
			}else {
			  int leaf=1;
			  for(int z=1;z<=4;z++){
				  int mistemp=mismatch;
				  if (get_index(decode(T,v))!=z) mistemp++;
				  if (curptr->ptr[z]!=NULL && mistemp<=mislimit){

					//curptr=curptr->ptr[z];
					leaf=0;
					//cout << "recurrsive call: "<< i <<" "<<j<<endl;
					all_pairs_helper(v+1,T,startpos,sorted,A,k,N,output,1,curpos+1,curptr->ptr[z],mistemp,i,j,next,mislimit);
				  }
			  }
			  return;
			}
